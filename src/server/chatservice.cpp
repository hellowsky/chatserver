...
void ChatService::login(const TcpConnectionPtr &conn, json &js, Timestamp time)
{
    ...
    if (user.getId() == id && user.getPwd() == password)
    {
        ...
        else
        {
            ...
            if (!vec.empty())
            {
                ...
                // 读取该用户的离线消息后，把该用户的所有离线消息删除，防止下次登录时之前的离线消息还在
    ...
    else
    {
        ...
        response["msgid"] = LOGIN_MSG_ACK;
        response["errno"] = 1;
        response["errmsg"] = "id or passwrod is invalid!";
        conn->send(response.dump());
    }
}
...
void ChatService::reg(const TcpConnectionPtr &conn, json &js, Timestamp time)
{
    ...
    {
        LOG_ERROR << "Missing required fields in registration JSON";
        return;
    }
    string name = js["name"];
    ...
    {
        // 数据库操作异常处理
        LOG_ERROR << "Error inserting user into database: " << e.what();
        json response;
        response["msgid"] = REG_MSG_ACK;
        response["errno"] = 2; // 假设2代表数据库操作错误
        conn->send(response.dump());
    }
}
...
void ChatService::chatGroup(const TcpConnectionPtr &conn, json &js, Timestamp time)
{
    int userid = js["id"].get<int>();
    int groupid = js["groupid"].get<int>();
    vector<int> usderidVec = _groupModel.queryGroupUsers(userid, groupid);
    lock_guard<mutex> lock(_userConnMutex);
    for (int id : usderidVec)
    {
